...
Material *SceneParser::parseMaterial()
{
    ...
    textureFilename[0] = 0;
    char cloudFilename[MAX_PARSER_TOKEN_LENGTH];
    cloudFilename[0] = 0;
    ...
    while (1)
    {
        ...
        {
            getToken(cloudFilename);
        }
        else if (strcmp(token, "normalMap") == 0)
        {
            getToken(normalMapFilename);
        }
        else if (strcmp(token, "disregardLighting") == 0)
        {
            disregardLighting = true;
        }
    ...
    answer->disregardLighting = disregardLighting;
    if (textureFilename[0] != 0)
    {
        answer->loadTexture(textureFilename);
    }
    if (cloudFilename[0] != 0)
    {
        answer->loadCloud(cloudFilename);
    }
    if (normalMapFilename[0] != 0)
    {
        answer->loadNormalMap(normalMapFilename);
    }
    if (noise != 0)
    {
...
Plane *SceneParser::parsePlane()
{
    ...
    Vector2f textureScale{1.0f, 1.0f};
    if (strcmp(token, "}") != 0)
    {
        assert(!strcmp(token, "textureScale"));
        textureScale = readVec2f();
        getToken(token);
    }

    assert(!strcmp(token, "}"));
    assert(current_material != NULL);
    return new Plane(normal, offset, current_material, textureScale);
}
